...
import { UtensilsCrossed, Package, Tag, ScrollText, ArrowLeft } from 'lucide-react'
...
export default function AdminSidebar({ isOpen, isMobile, onClose }: AdminSidebarProps) {
  ...
  const menuItems = [
    { path: '/admin/products', label: 'Products', icon: UtensilsCrossed },
  ...
  return (
    <>
      ...
      <aside className={clsx(styles.sidebar, isOpen && styles.open, !isOpen && !isMobile && styles.closedDesktop, isMobile && styles.mobile)}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={clsx(styles.item, location.pathname.startsWith(item.path) && styles.active)}
