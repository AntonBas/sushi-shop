    ...
    <div className={styles.page}>
      ...
      {activeTab === 'profile' && (
        <form onSubmit={handleUpdateProfile} className={styles.form}>
          ...
          <Input label="Phone" value={phone} onChange={setPhone} placeholder="+380991234567" />
          <Button type="submit" loading={updateApi.loading}>Save</Button>
        </form>
      )}
      ...
      {activeTab === 'address' && (
        <form onSubmit={handleUpdateAddress} className={styles.form}>
          ...
          <Button type="submit" loading={updateApi.loading}>Save Address</Button>
        </form>
      )}

      {activeTab === 'password' && (
        <form onSubmit={handleChangePassword} className={styles.form}>
          <Input label="Current Password" type="password" value={oldPassword} onChange={setOldPassword} placeholder="••••••••" />
          <Input label="New Password" type="password" value={newPassword} onChange={setNewPassword} placeholder="Min 8 characters" />
          <Button type="submit" loading={passwordApi.loading}>Change Password</Button>
        </form>
      )}
    </div>
  )
